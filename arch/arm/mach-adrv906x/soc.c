...
#define ADRV906X_LOADCMD \
	"run adrv906x_fit_loadcmd; run adrv906x_secondary_loadcmd;"

#define ADRV906X_SEC_LOADCMD \
	"setenv sec_loadcmd_fail 0 && fdt addr ${adrv_bootaddr} && " \
	...
	"setenv numbytes ${sec_initrd_size} && setenv src ${sec_initrd_src_addr} && " \
	"setenv dst ${sec_initrd_dst_addr}; if test $? -ne 0; then setenv sec_loadcmd_fail 1; fi; " \
	"test ${sec_loadcmd_fail} -eq 0 && while test ${numbytes} -ge 0x100000; " \
	"do cp.b ${src} ${dst} 0x100000 && " \
	"setexpr numbytes ${numbytes} - 0x100000 && " \
	"setexpr src ${src} + 0x100000 && " \
	"setexpr dst ${dst} + 0x100000 && " \
	"setenv numbytes 0x${numbytes}; " \
	"if test $? -ne 0; then setenv sec_loadcmd_fail 1; " \
	"setenv numbytes 0; fi; done; test ${sec_loadcmd_fail} -eq 0 && " \
	"cp.b ${src} ${dst} ${numbytes} && " \
	...
	"fdt addr ${sec_fdt_dst_addr} && fdt resize && " \
	...
	"setexpr a2 ${regval} / 10000 && setexpr a2 ${a2} % 100 && " \
	"setexpr a2 ${a2} * 100 && setexpr a3 ${regval} / 100 && " \
	"setexpr a3 ${a3} % 100 && setexpr a3 ${a3} * 10000 && setexpr a4 ${regval} % 100 && " \
	"setexpr a4 ${a4} * 1000000 && setexpr res_size ${a1} + ${a2} && " \
	...
	"mw.q ${addr} ${sec_fdt_dst_addr} && setexpr addr ${sec_memory_base} + 0x8 && " \
	"mw.q ${addr} ${sec_kernel_dst_addr} && mw.q ${sec_memory_base} 0xAD12B007 && " \
	"dcache flush; if test $? -ne 0; then setenv sec_loadcmd_fail 1; fi; " \
	"if test ${sec_loadcmd_fail} -eq 1; then " \
	"echo Error: Secondary loading failed; " \
	"fdt addr ${fdtcontroladdr}; fdt resize; " \
	"fdt rm /boot dual-tile; fdt rm /boot secondary-linux-enabled; " \
	"fi;"
...
static int arch_misc_init_adrv906x(void)
{
	...
	if (ret == 0 && is_secondary_linux_enabled == 1) {
		...
		env_set_ulong("sec_sysclk_freq", (u32)get_sysclk_freq());
		 ...
		 */
		env_set("adrv906x_secondary_loadcmd", ADRV906X_SEC_LOADCMD);
