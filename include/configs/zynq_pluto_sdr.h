 ...
 *
 * Configuration for the Analog Devices PlutoSDR and M2K boards
 */
...
/*
 * Env version: increment when boot scripts change in incompatible ways.
 * On boot, if saved env_version != this, reset to defaults but preserve
 * user-customizable variables via the extraenv area at 0xFF000.
 */
#define PLUTO_ENV_VERSION "2"

#define CFG_EXTRA_ENV_SETTINGS \
	"env_version=" PLUTO_ENV_VERSION "\0" \
	"modeboot=qspiboot\0" \
	...
	"qspiboot=setenv silent 1;adi_hwref;test -n $PlutoRevA || gpio input 14 && setenv silent && sf probe && sf protect lock 0 100000 && run dfu_sf;  " \
		"setenv silent;" \
